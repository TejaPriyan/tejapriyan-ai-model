...
| I want to… | Go to |
|---|---|
| **Chat in terminal instantly (zero install)** | [Option 0](#option-0--instant-terminal-chat-via-npx-0-minutes) |
...
### Option 0 — Instant Terminal Chat via NPX (0 minutes)

You don't need to install or download anything. Open your terminal and run:
...
```bash
npx tejapriyan "What is 15% of 200?"
```

- Connects automatically to your local Ollama if running on `localhost:11434`.
- Falls back to built-in offline intelligence if Ollama is not started.
- Type `/help`, `/setup`, or `/web` inside the chat for more options.
...
> 💡 **Quick Alternative (Ollama v0.1.33+):** If your Ollama supports direct Hugging Face pulls, you can also run:
> ```bash
> ollama run hf.co/teja161615/Tejapriyan-8B-GGUF
> ```
> However, downloading the GGUF directly using the `curl` commands above is the most reliable method across all setups.
