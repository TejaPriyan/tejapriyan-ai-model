...
function solveIdentity(q: string, history: ChatMessage[]): string | null {
  ...
  if (lower.includes("how") && (lower.includes("run") || lower.includes("use") || lower.includes("install") || lower.includes("download") || lower.includes("ollama"))) {
    return `You can run me privately on your computer with Ollama:

\`\`\`bash
# 1. Download the GGUF weights directly
...
You can also download weights directly from Hugging Face at [teja161615/Tejapriyan-8B-GGUF](https://huggingface.co/teja161615/Tejapriyan-8B-GGUF).`;
  }
